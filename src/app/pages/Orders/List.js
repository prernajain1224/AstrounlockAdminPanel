...
const OrdersList = () => {
  ...
  const { apiData, isLoading, serverError } = useFetch(ordersList, {});
  ...
  const columns = [
    ...
    { name: "Payment Method", selector: (row) => row?.payment_method },
    {
      ...
      selector: (row) => (row?.payment_received ? "Yes" : "No")
    },
    { name: "Wallet Balance Use", selector: (row) => formatPrice(row?.wallet_balance_used) },
    { name: "GST", selector: (row) => formatPrice(row?.gst) },
    { name: "Order Total", selector: (row) => formatPrice(row?.total) },
  ...
  return (
    <Layout>
      <>
        ...
        <p className="mb-4">Showing the list of Orders</p>
