...
const WalletsList = () => {
  ...
  const { apiData, isLoading, serverError } = useFetch(WalletBalanceList, {});
  ...
  const columns = [
    { name: "ID", selector: (row) => row?.id ?? row?._id, sortable: false },
    {
      name: "User Name",
      cell: (row) => (
        <Link to={`/astrologer/${row?.user?.id}/view`}>
          {row?.user?.name}
        </Link>
      )
    },
    { name: "Balance", selector: (row) => formatPrice(row?.balance) },
  ...
  return (
    <Layout>
      <>
        <h1 className="h3 mb-2 text-gray-800">Wallets</h1>
        <p className="mb-4">Showing the list of Wallets</p>
              ...
              <div className="table-responsive">
                {apiData?.data?.length > 0 ? (
                  <ListingTable
                    ...
                    data={apiData?.data}
                    isLoading={isLoading}
                    filterColumn={["id", "balance"]}
                  />
                ) : null}
              </div>
            )}
            {/* <Link
              to={`${RoutesList.categorie}/new`}
