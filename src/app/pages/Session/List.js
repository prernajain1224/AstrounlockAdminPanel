...
const SessionList = () => {
  ...
  const columns = [
    ...
    { name: "Session Name", selector: (row) => row?.product_name },
    { 
         name: "Base Price", 
         selector: (row) => formatPrice(row?.base_price)
    },
    { name: "Category", selector: (row) => row?.category_name },
     { name: "Status", selector: (row) => row?.status },
    { name: "Session Type", selector: (row) => row?.session_type },
  ...
  return (
    <Layout>
      <>
        <h1 className="h3 mb-2 text-gray-800">Session Section</h1>
        <p className="mb-4">Showing the list of Session Section</p>
            ...
            {isLoading ? (
              ...
              <div className="table-responsive">
                {apiData?.data?.length > 0 ? (
                  <ListingTable
                    columns={columns}
                    data={apiData?.data?.filter((item) => item?.product_type === "Session")}
                    isLoading={isLoading}
