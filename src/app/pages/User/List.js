...
const UserList = () => {
  ...
  const columns = [
    ...
    // { name: "phone_number", selector: (row) => row?.phone_number },
  ...
  return (
    <Layout>
      <>
        ...
        <p className="mb-4">Showing the list of User</p>
              ...
              <div className="table-responsive">
                {apiData?.data?.length > 0 ? (
                  <ListingTable
                    columns={columns}
                    data={apiData?.data}
                    // data={apiData?.data?.filter((user) => user.user_type === "User")}
                    isLoading={isLoading}
