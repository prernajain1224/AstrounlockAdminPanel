...
const BlogsList = () => {
  ...
  const { apiData, isLoading, serverError } = useFetch(blogsList, {});

  const { makeRequest, data, isSubmitLoading, submitError } =
    useSubmit(deleteBlogs);
  ...
  return (
    <Layout>
      <>
        <h1 className="h3 mb-2 text-gray-800">Blogs</h1>
        <p className="mb-4">Showing the list of Blogs</p>
              ...
              <div className="table-responsive">
                {apiData?.data?.length > 0 ? (
                  <ListingTable
                    ...
                    data={apiData?.data}
                    isLoading={isLoading}
                    filterColumn={["id","title","post_by_name"]}
                  />
                ) : null}
              </div>
            )}
            <Link
              to={`${RoutesList.blogs}/new`}
