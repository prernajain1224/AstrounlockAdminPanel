  ...
  return (
    <Layout>
      <div className="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
          <h1 className="h3 text-gray-800">Product Details</h1>
          <p className="mb-0">
            {isLoading ? "Loading" : "Showing"} details of Product
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className="table-responsive">
                {!hasError ? (
                  <>
                    {detailRow("Product ID", productData?.data?.id)}
                    {detailRow("Name", productData?.data?.product_name)}
                    {detailRow("Description", productData?.data?.description, true)}
                    {detailRow("Price", formatPrice(productData?.data?.price))}
                    {detailRow("Tagline", productData?.data?.tagline)}
                    ...
                    {detailRow("Stock Available", productData?.data?.stock_available)}
                    {detailRow("Created At", formatDateTime(productData?.data?.created_date))}
                    {detailRow("Banner", productData?.data?.banner)}
                    {console.log("ddddddddddd", getImageUrl(productData?.data?.banner))}
